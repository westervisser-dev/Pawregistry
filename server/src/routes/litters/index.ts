...
export const littersRoutes = new Elysia({ prefix: '/litters' })
	...
	.post('/:id/interest', async ({ params, user, error }) => {
		...
		if (!litter) return error(404, { error: 'Not found', message: 'Litter not found' });
		if (litter.status === 'planned') return error(400, { error: 'LitterNotOpen', message: 'You cannot mark interest in a litter that is not yet open' });
	...
	.post(
		...
		},
		{
			body: t.Object({
				...
				estimatedAdultWeightMinKg: t.Optional(t.Nullable(t.Number())),
				estimatedAdultWeightMaxKg: t.Optional(t.Nullable(t.Number())),
				estimatedAdultHeightMinCm: t.Optional(t.Nullable(t.Number())),
				estimatedAdultHeightMaxCm: t.Optional(t.Nullable(t.Number())),
	...
	.patch(
		...
		{ body: t.Partial(t.Object({
			...
			estimatedAdultWeightMinKg: t.Nullable(t.Number()),
			estimatedAdultWeightMaxKg: t.Nullable(t.Number()),
			estimatedAdultHeightMinCm: t.Nullable(t.Number()),
			estimatedAdultHeightMaxCm: t.Nullable(t.Number()),
