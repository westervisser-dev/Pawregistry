...
export function PortalChecklist() {
	...
	return (
		<div>
			<div className="mb-8">
				<h1 className="font-serif text-2xl font-bold text-warm-900">Go-Home Checklist</h1>
				<p className="text-warm-600 text-sm mt-1">Everything that needs to happen before pickup day.</p>
			</div>

			{!checklist ? (
				<Card className="p-12 text-center">
					<p className="text-4xl mb-4" aria-hidden="true">✅</p>
					<p className="text-warm-600 font-medium">No checklist yet</p>
					<p className="text-warm-400 text-sm mt-1">Your breeder will set this up once you're matched.</p>
				</Card>
			) : (
				<Card className="p-6">
					<div className="flex items-center justify-between mb-6">
						<span className="text-sm text-warm-500">{completed} of {items.length} complete</span>
						<div className="flex-1 mx-4 bg-warm-100 rounded-full h-2 overflow-hidden">
							<div
						...
						{items.map(({ label, done }) => (
							<div key={label} className="flex items-center gap-3">
								<div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${
									done ? 'bg-green-100 text-green-600' : 'bg-warm-100 text-warm-300'
								}`} aria-hidden="true">
									{done ? '✓' : '○'}
								</div>
								<span className={`text-sm ${done ? 'text-warm-500 line-through' : 'text-warm-800'}`}>
									{label}
								</span>
							</div>
						))}
					...
					{checklist.goHomeDate && (
						<div className="mt-6 pt-4 border-t border-black/[0.05]">
							<p className="text-sm text-warm-500">
								Go-home date: <span className="font-medium text-warm-900">{checklist.goHomeDate}</span>
							</p>
						</div>
					)}
				</Card>
